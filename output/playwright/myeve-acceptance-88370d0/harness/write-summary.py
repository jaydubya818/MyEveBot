...
out=Path('/Users/jaywest/Myeve/output/playwright/myeve-acceptance-88370d0')
root=Path('/tmp/myeve-acceptance-88370d0')
report='''# MyEve / Sofie local product acceptance
...
[All 251 requested sections](COVERAGE.md) · [Machine-readable coverage and original requirements](coverage.json) · [Evidence directory](evidence/) · [Source excerpts at the tested SHA](source/)
...
shutil.copytree(root/'evidence',out/'evidence',dirs_exist_ok=True)
(out/'harness').mkdir(exist_ok=True)
for name in ['local-transport.mjs','migrate-local.mjs','control-fixtures.mjs','isolated-pg-loader.mjs','isolated-pg.mjs','archive-cases.py','build-report.py','write-summary.py']:
 shutil.copy2(root/name,out/'harness'/name)
for file in ['app/chat.tsx','components/results-panel.tsx','components/agents-panel.tsx','components/computer-viewer.tsx','components/files-page.tsx','lib/chat-file-client.ts','lib/owner-data.ts','lib/web-auth.ts','app/api/computer-profiles/route.ts']:
 dest=out/'source/apps/eve'/file;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(root/'apps/eve'/file,dest)
print(out/'REPORT.md')
