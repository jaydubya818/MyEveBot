...
First beta admission is limited by a configured maximum active tester count. Every new Eve requires an operator decision. Model work has a per-Eve spending ceiling enforced by the Eve runtime, while Vercel's team spend management supplies the infrastructure backstop; Vercel does not offer a dependable per-project hard cost cap through the project API. The control plane shows both the configured ceiling and the latest observed use, and pauses when metering is unavailable or a limit is reached.
