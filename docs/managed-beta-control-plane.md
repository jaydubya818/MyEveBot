...
The first control-plane slice issues private invitation links, provisions one isolated project, lists environments, checks project identity and health, verifies the AI budget, upgrades a ready Eve from the Builder template, pauses or resumes a ready Eve, and records an operator-checked owner export hash. A failed operation remains visible with its stage. Revocation, scheduled monitoring, and retirement still need implementation and qualification before the first tester is invited.
