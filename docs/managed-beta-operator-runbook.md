...
For a setup that failed before any deployment or storage connection, `POST /api/managed/environments/{id}/recover` with the admin bearer token and `{ "confirmProjectName": "<exact-recorded-project-name>", "confirmEmptyProjectDeletion": true }`. The retirement switch must be enabled. Recovery checks the exact project ID, absence of deployments, and absence of connected or similarly named storage before deleting and verifying the empty project. It retires the failed record so a new invitation can be issued without duplicating resources. If any check fails, inspect provider resources manually; a failed Eve with data or an uncertain store must use the checked export and retirement path.
