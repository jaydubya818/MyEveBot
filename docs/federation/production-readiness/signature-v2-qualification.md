...
[Live results](evidence/signature-v2/kms-live.json), [reproducible probe](https://github.com/jaydubya818/relay/blob/c910c9d/scripts/qualify-signature-v2.mjs), [regression evidence](evidence/signature-v2/regressions.json), [performance](evidence/signature-v2/browser-performance.json), [local simulation](evidence/signature-v2/three-worker-simulation.json).
...
Final Relay candidate/evidence commit: `c910c9d` on `codex/myeve-federation-preprovision`, remote parity verified. Full MyEve candidate remains unpublished due to the missing ancestor; the complete migration patch is preserved in this branch. The original failed raw probe is unchanged.
