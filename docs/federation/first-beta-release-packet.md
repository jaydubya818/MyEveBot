...
| Managed Eve Control Plane | Operator-only registry and exact-ID provisioning, $1 rehearsal budget, archive export/verification, monitor, pause/resume, and one backed-up Orchis upgrade passed. A second disposable Eve on release 262 passed the general owner-authorized delete path: fresh 38-file export, exact permit, Relay retirement, project and Neon removal, tombstone, and public 404. Orchis has a dedicated project/database and $20/month AI Gateway project budget. Paired hosted Relay retirement remains untested | Guided beta; paired retirement check pending |
...
2. Completed: Orchis's isolated Eve is at `https://orchis-eve-beta.vercel.app`. Owner sign-in, model reply, archive validation (38 files), Relay setup form, and post-upgrade monitor passed. Full resource IDs, export hashes, and credentials are in protected operator files. Upgrade Orchis to release 262 and repeat owner readiness before issuing the invitation.
