...
| --- | --- | --- |
| Relay invitation and separate owner | Invitation code is merged into Relay main; a synthetic signup created a separate account and denied replay. PR #24 keeps the token out of the URL query. Production's 24 migration hashes match through `0023`, and `0024` passed a local rehearsal; the invitation release is not deployed | Blocked |
...
| MyEve Builder | Current main plus PR #32 passes pairing tests, typecheck, manifest and build. The Builder project has a Ready production deployment, but its alias redirects unauthenticated users to Vercel SSO and its source SHA is unverified | Blocked |
| MyEve Agent dependency review | `langsmith@0.3.87` remains in the transitive tree. The high advisory requires an untrusted public prompt pull; the Agent source has no such call, and Builder does not set optional `RAINDROP_WRITE_KEY` | Scoped exception for this guided beta only; reopen if telemetry is enabled |
...
2. Back up the production Relay database and repeat the read-only lineage check in the deployment window. A September 26 audit found exactly 24 matching migration hashes/timestamps through `0023_channel_execution`, no `beta_invites` table, and baseline counts of 3 accounts, 3 users, and 2 agents. A disposable database built from those 24 SQL files accepted `0024_beta_invites` cleanly; manual production schema drift remains unexcluded. Keep `RELAY_ALLOW_SIGNUP=false`. Set `RELAY_BETA_INVITER_EMAILS` to the exact pilot operator email and confirm the existing owner can still sign in while unauthorized owners cannot issue invitations. The old `RELAY_REQUIRE_INVITE` and `RELAY_TESTER_INVITE_ISSUER_EMAIL` settings from a different local branch do not control current main.
3. Qualify MyEve Builder from the monorepo root with a Builder-only build and `apps/builder/.next` output. The separate `jaydubya818/myeve-builder` Vercel project now has a Ready production deployment (`dpl_H3U7KPbdFY7pNih4UK9N9V24LKmq`) aliased to `https://myeve-builder-jaydubya818.vercel.app`, but an unauthenticated `/api/template-version` request redirects to Vercel SSO, and deployment metadata does not establish the source SHA. Do not share it yet. Verify the exact release SHA and target; avoid another CLI deployment until the project's production/preview classification is understood. [Builder PR #32](https://github.com/jaydubya818/MyEveBot/pull/32) makes `BUILDER_RELAY_ORIGIN` an exact operator-configured HTTPS origin; current main's hardcoded origin returns 404. Make Relay's `/api/federation/trust` publicly reachable without Vercel SSO, supply its signing-key fingerprint to the tester through an independent trusted channel, and have Builder verify that fingerprint before creating the tester's project. The intended Relay alias currently redirects unauthenticated requests to Vercel SSO. Verify `/api/template-version` publicly and complete a fresh account's deployment before trusting the URL.
...
Current MyEve main plus Builder PR #32 passed five pairing tests, typecheck, the release manifest, and a webpack production build. MyFactory's hydrated checkout passed its full test suite, typecheck, and production build after increasing a process-start fixture's timeout from 200 ms to three seconds; [MyFactory PR #1](https://github.com/jaydubya818/MyFactory/pull/1) carries that test-only fix for review. The production dependency audit still reports `langsmith@0.3.87` through `raindrop-ai`. The [high-severity public prompt advisory](https://github.com/advisories/GHSA-3644-q5cj-c5c7) requires calling `pullPrompt`/`pullPromptCommit` with an untrusted public prompt; no such call exists in the Agent or Builder source. Builder does not provision `RAINDROP_WRITE_KEY`, so the optional Raindrop client is not instantiated in its new Agent by default. This is a scoped exception for one guided beta if that key stays absent, not a dependency fix or a claim that all telemetry use is safe. Upgrade the dependency path before broad rollout. These checks do not replace hosted acceptance.
...
| Builder origin | Candidate `https://myeve-builder-jaydubya818.vercel.app`; **not shareable until public access and source SHA are verified** |
