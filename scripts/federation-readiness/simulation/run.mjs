...
async function wait(fn,ms=90000){const end=Date.now()+ms;let last;while(Date.now()<end){try{const value=await fn();if(value)return value;}catch(e){last=e;}await sleep(200);}throw last??Error('Local condition timeout');}
...
try{
 ...
 const complete=async(worker,id)=>wait(async()=>{const r=await command(worker,'get',undefined,id);return ['COMPLETED','REJECTED','DENIED'].includes(r.status)?r:null;});
