...
export async function startController({cwd=process.cwd(),environment=process.env,modelRequest}={}){
 ...
 try{
  ...
  const model=haikuQualificationModel({authority,credential:environment.ANTHROPIC_API_KEY,pricingReviewedUntil:Number(environment.FQ_PRICING_REVIEWED_UNTIL),...(modelRequest?{request:modelRequest}:{})});
