...
export async function runWorker({component,cwd=process.cwd(),environment=process.env,request=fetch}){
 ...
 try{return await supervise({command,cwd:component==='relay'?cwd:`${cwd}/apps/eve`,environment:childEnv,sourceSha:sha,heartbeat,lifetimeMs:3600000});}finally{health?.close();}
}
