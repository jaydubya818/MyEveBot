// Operator-only bounded synthetic probe. No resource creation, retries or protocol changes.
...
assert.equal(sha,'d817c848a7f6797fe04787e28ac96081b69359ae');
...
const fixtures=[
 ...
 {name:'work_intermediate',envelope:{...base,capability:'work.request',payload:{category:'artifact_generation',task:'\0'.repeat(4000),expectedOutput:'\0'.repeat(1000),context:Array(10).fill('\0'.repeat(255)),budget:{runtimeSeconds:3600,cost:'999999.999999999',modelSteps:100,delegatedWorkers:0},deadline:expiresAt}}},
 {name:'escaped_intermediate',envelope:{...base,capability:'message.send',payload:{subject:'\0'.repeat(200),body:'\0'.repeat(16000),replyTo:'\0'.repeat(255)}}},
];
const materialFor=(envelope,keyId)=>`${Buffer.from(canonicalJson({alg:'EdDSA',typ:'relay-federation+jwt',kid:keyId})).toString('base64url')}.${Buffer.from(canonicalJson({iss:issuer,aud:audience,jti:base.id,iat:Math.floor(Date.now()/1000),exp:Math.floor(Date.now()/1000)+60,envelope})).toString('base64url')}`;
...
for(const fixture of fixtures){
 ...
 fixture.rawBytes=Buffer.byteLength(captured);fixture.base64Bytes=Buffer.from(captured).toString('base64').length;fixture.localCanonicalVerification=true;
}
const report={observedAt:new Date().toISOString(),relaySha:sha,keyVersion:version,status:'NOT_RUN',federationEnabled:false,externalGates:{independentSecurity:'NOT_RUN',productionPlatform:'NOT_RUN'},cases:fixtures.map(({name,rawBytes,base64Bytes,localCanonicalVerification})=>({name,rawBytes,base64Bytes,localCanonicalVerification,kms:'NOT_RUN',relayVerification:'NOT_RUN'})),requests:[]};
if(process.argv.includes('--prepare-only')){console.log(JSON.stringify(report,null,2));process.exit(0);}
assert.equal(process.env.FQ_KMS_PROBE_AUTHORIZED,'qualification-only-new-key');
...
for(let i=0;i<fixtures.length;i++){
 ...
 try{
  ...
  await verifyDelivery(signed,verification(pub.pem));result.relayVerification='PASS';result.signedInputSha256=createHash('sha256').update(captured).digest('hex');result.tokenBytes=signed.length;
