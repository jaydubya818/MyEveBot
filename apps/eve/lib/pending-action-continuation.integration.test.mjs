...
suite('canonical owner pending-action continuation',()=>{
 ...
 it('signed work dispatches once and resumes its exact Action after worker restart',async()=>{
  ...
  const adapter={resolveTarget:async()=>({provider:'fixture',account:mapping.ownerId,resource:'/workspace/fixture.txt'}),execute:async(parameters,context)=>{await consumeActionAuthority(context,parameters,'files.write');await consumeProviderAuthority(context,parameters,'files.write');effects++;return {};},verify:async()=>({verified:true,receipt:{id:'fixture'}})};
  injected.session={state:{sessionId:admitted.runId},send:async()=>{
   const claim=verifyOwnerRuntime(injected.clientOptions.headers()['x-myeve-owner-run']);await bindOwnerRuntime(claim,admitted.runId,'first-turn',database);
   research++;draft++;
 ...
 it('expired canonical Run denies delayed approval without extending its deadline',async()=>{
  const input=work();input.taskId=input.requestId;const service=new OwnerChannelHandoff(trust,database);const admitted=await service.accept(envelope({commandId:'start-'+input.requestId,operation:'start',work:input}));
  await query("UPDATE task_runs SET status='running',deadline_at=now()+interval '60 seconds' WHERE id=$1",[admitted.runId]);
  await query("INSERT INTO task_run_sessions(task_id,session_id,role) VALUES($1,$1,'orchestrator')",[admitted.runId]);
  ...
  await expect(new OwnerRunControl(database).apply(await service.accept(envelope(approval)),()=>adapter)).rejects.toThrow('stale');
  const [after]=await query('SELECT deadline_at::text AS deadline FROM task_runs WHERE id=$1',[admitted.runId]);
  expect(after.deadline).toBe(before.deadline);expect(effects).toBe(0);
  expect((await query('SELECT budget_resumed_at FROM owner_channel_requests WHERE run_id=$1',[admitted.runId]))[0].budget_resumed_at).toBeNull();
 });
