...
function daysUntil(value: string, now: Date): number | null {
  ...
  return difference < 0 ? -1 : Math.ceil(difference / DAY_MS);
}
...
export function detectGoalRisks(
  ...
  for (const goal of goals) {
    ...
    if (goal.status === "blocked") {
      risks.push(risk(goal, null, "work_blocked", "warning", "The goal is explicitly blocked."));
    }
    ...
    for (const task of goal.tasks) {
      ...
      if (task.status === "blocked") {
        risks.push(risk(goal, task, "work_blocked", "warning", "This task is explicitly blocked."));
      } else if (task.status === "failed") {
        risks.push(risk(goal, task, "task_failed", "critical", "This task failed and needs an explicit recovery path."));
      }
    ...
    if (goal.tasks.length > 0 && staleDays >= 14 && activelyExpected) {
      risks.push(risk(
        goal,
        null,
        "no_recent_progress",
        "attention",
        `No persisted progress was recorded for ${staleDays} days.`,
      ));
    }
  }
  return risks;
...
export function buildDailyBrief(goals: readonly GoalDetailView[], now = new Date()): DailyBriefView {
  ...
  const tasks = operatingGoals.flatMap((goal) => goal.tasks.map((task) => ({ goal, task })));
  const unfinished = tasks.filter(({ task }) => !TERMINAL_TASKS.has(task.status));
  const overdueGoals = operatingGoals.filter((goal) => goal.targetDate !== null && (daysUntil(goal.targetDate, now) ?? 0) < 0);
  const approachingGoals = operatingGoals.filter((goal) => goal.targetDate !== null && (daysUntil(goal.targetDate, now) ?? 99) >= 0 && (daysUntil(goal.targetDate, now) ?? 99) <= 7);
  return {
    kind: "daily",
...
export function buildWeeklyReview(
  ...
  return {
    ...
    blockers: risks.filter((value) => ["work_blocked", "task_failed", "dependency_incomplete", "capability_unavailable"].includes(value.reason)),
    missedCommitments: operatingTasks.filter(({ task }) => !TERMINAL_TASKS.has(task.status) && task.dueAt !== null && (daysUntil(task.dueAt, now) ?? 0) < 0).map(({ goal, task }) => item(goal, task, task.dueAt)),
