...
const started = { type: "session.started", data: {}, meta: { id: crypto.randomUUID(), at: "2026-09-20T00:00:00Z" } } as HandleMessageStreamEvent;
const received = { type: "message.received", data: { message: "Hello", turnId: "turn_0", sequence: 0 }, meta: { id: crypto.randomUUID(), at: "2026-09-20T00:00:01Z" } } as HandleMessageStreamEvent;
const waiting = { type: "session.waiting", data: { continuationToken: "fresh", wait: "next-user-message" }, meta: { id: crypto.randomUUID(), at: "2026-09-20T00:00:02Z" } } as HandleMessageStreamEvent;
describe("saved chat stream cursor", () => {
  it("advances an old cursor past the already-rendered turn", () => {
    const fixed = reconcileChatSession({ events: [started, received, waiting], session: { sessionId: "s", streamIndex: 1 } });
    expect(fixed.session).toEqual({ sessionId: "s", streamIndex: 3 });
  });
  ...
  it("keeps repeated prompts in distinct turns", () => {
    const second = { ...received, data: { message: "Hello", turnId: "turn_1", sequence: 1 }, meta: { id: crypto.randomUUID(), at: "2026-09-20T00:01:00Z" } } as HandleMessageStreamEvent;
  ...
  it("counts from the current session rather than older conversation history", () => {
    const next = { ...started, meta: { id: crypto.randomUUID(), at: "2026-09-21T00:00:00Z" } };
  ...
  it("does not guess a cursor from partial logs or resurrect terminal sessions", () => {
    ...
    const ended = { events: [started, waiting], session: undefined };
