...
async function loadRoutineHistory(context: DomainContext): Promise<OwnerDataCategory> {
  const [reminders, routines, versions, occurrences, attempts, deliveries] = await Promise.all([
    ...
    ownerRows(context, `SELECT id,routine_id,routine_version,occurrence_key,scheduled_for,run_id,status,attempt_count,failure_category,completed_at,cost_status,cost_usd,created_at,updated_at FROM execution_occurrences WHERE owner_id=$1 ORDER BY scheduled_for,id`),
