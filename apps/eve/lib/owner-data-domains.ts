...
// Preserve transcripts while excluding legacy channel continuation authority.
// Arbitrary secret-bearing user fields still fail archive validation.
function archiveConversation(row: OwnerDataRow): OwnerDataRow {
  if (!row.chat || typeof row.chat !== "object" || Array.isArray(row.chat)) return row;
  const chat = { ...row.chat } as Record<string, unknown>;
  if (chat.session && typeof chat.session === "object" && !Array.isArray(chat.session)) {
    const { continuationToken: _token, ...session } = chat.session as Record<string, unknown>;
    chat.session = session;
  }
  if (Array.isArray(chat.events)) chat.events = chat.events.map(event => {
    if (event?.type !== "session.waiting" || !event.data || typeof event.data !== "object") return event;
    const { continuationToken: _token, ...data } = event.data;
    return { ...event, data };
  });
  return { ...row, chat };
}
...
async function loadActionHistory(context: DomainContext): Promise<OwnerDataCategory> {
  const [requests, receipts] = await Promise.all([
    ownerRows(context, `SELECT id,run_id,occurrence_id,action_key,executor,trigger,capability_id,action_class,target,safe_summary,decision,authority_source,approval_id,status,attempt_count,computer_session_id,control_version,reason_code,policy_version,created_at,updated_at FROM action_requests WHERE owner_id=$1 ORDER BY created_at,id`),
    ownerRows(context, `SELECT action_id,attempt_number,event,created_at FROM action_receipts WHERE owner_id=$1 ORDER BY created_at,id`),
  ]);
  const historical = (row: OwnerDataRow) => ({ ...row, authority: "historical_only", restorableAuthority: false });
  return category(
    OWNER_DATA_DOMAINS_BY_ID.action_history,
    { requests: requests.map(historical), receipts: receipts.map(historical) },
    "metadata_only",
    "non_restorable",
...
async function loadRoutineHistory(context: DomainContext): Promise<OwnerDataCategory> {
  const [reminders, routines, versions, occurrences, attempts, deliveries] = await Promise.all([
    ownerRows(context, `SELECT id,prompt,cron,timezone,next_fire_at,status,created_at,last_fired_at,routine_name,approval_boundary,source_outcome_id,configuration_version,reviewed_version,reviewed_at,execution_routine_id FROM reminders WHERE owner_id=$1 ORDER BY created_at,id`),
    ownerRows(context, `SELECT id,source_kind,source_id,name,agent_id,version,status,configuration,consecutive_failures,failure_threshold,last_success_at,paused_at,pause_sequence,created_at,updated_at FROM execution_routines WHERE owner_id=$1 ORDER BY created_at,id`),
    ownerRows(context, `SELECT routine_id,version,configuration,changed_by,created_at,review_binding FROM execution_routine_versions WHERE owner_id=$1 ORDER BY routine_id,version`),
    ownerRows(context, `SELECT id,routine_id,routine_version,occurrence_key,scheduled_for,run_id,status,admission,preflight,attempt_count,failure_category,completed_at,cost_status,cost_usd,created_at,updated_at FROM execution_occurrences WHERE owner_id=$1 ORDER BY scheduled_for,id`),
    ownerRows(context, `SELECT occurrence_id,attempt_number,status,failure_category,retry_decision,started_at,finished_at,cost_status,cost_usd FROM execution_attempts WHERE owner_id=$1 ORDER BY occurrence_id,attempt_number`),
    ownerRows(context, `SELECT id,occurrence_id,run_id,requested_channel,status,attempt_count,next_attempt_at,result_reference,result_unknown,created_at,updated_at FROM review_deliveries WHERE owner_id=$1 AND occurrence_id IS NOT NULL ORDER BY created_at,id`),
  ]);
  const inactive = (row: OwnerDataRow) => ({ ...row, restoreStatus: "disabled_needs_owner_review", restorableAuthority: false });
  return category(
    OWNER_DATA_DOMAINS_BY_ID.routines,
    {
      reminders: reminders.map(inactive),
      routines: routines.map(inactive),
      versions: versions.map(inactive),
      occurrences: occurrences.map((row) => ({ ...row, authority: "historical_only", restorableAuthority: false })),
      attempts: attempts.map((row) => ({ ...row, authority: "historical_only", restorableAuthority: false })),
      deliveries: deliveries.map((row) => ({ ...row, authority: "historical_only", restorableAuthority: false })),
    },
    "complete",
...
export const OWNER_DATA_DOMAINS: readonly OwnerDataDomain[] = [
  ...
  { id: "routines", name: "Routines & schedules", description: "Owner-scoped Routine definitions, versions, occurrences, attempts, and delivery history", exportable: true, restorable: false, deletable: true, sensitivity: "sensitive", dependencies: ["agents", "runs", "results"], required: true, load: loadRoutineHistory },
  ...
  { id: "action_history", name: "Action history", description: "Action Gateway decisions, safe targets, attempts, and receipt timeline without executable authority", exportable: true, restorable: false, deletable: true, sensitivity: "sensitive", dependencies: ["agents", "runs", "approval_history"], required: false, load: loadActionHistory },
  ...
  { id: "peer_permissions", name: "Peer permissions", description: "Owner policy metadata; no Relay credentials or executable authority", exportable: true, restorable: false, deletable: false, sensitivity: "sensitive", dependencies: ["agents"], required: false,
    load: async c => category(OWNER_DATA_DOMAINS_BY_ID.peer_permissions, {
      relationships: (await ownerRows(c, `SELECT id,local_agent_id,relay_origin,local_relay_account_id,local_relay_agent_id,peer_account_id,peer_agent_id,display_name,policies,expires_at,revoked_at,revision,created_at,updated_at FROM myeve_peer_permissions WHERE owner_id=$1 ORDER BY id`))
        .map(row => ({ ...row, restorableAuthority: false, restoreStatus: "requires_fresh_owner_review" })),
    }, "metadata_only", "non_restorable", ["Peer relationships are reference metadata only. Restore never reactivates policy, grants, credentials or pending Actions. Reestablish each exact relationship through owner review."]) },
