...
export const OWNER_DATA_DOMAINS: readonly OwnerDataDomain[] = [
  ...
  { id: "routines", name: "Routines & schedules", description: "Reminders, routines, safe trigger definitions, and review delivery preferences", exportable: true, restorable: false, deletable: true, sensitivity: "standard", dependencies: ["results"], required: true, load: async (c) => category(OWNER_DATA_DOMAINS_BY_ID.routines, { reminders: await c.query(`SELECT id,prompt,cron,timezone,next_fire_at,status,created_at,last_fired_at,routine_name,approval_boundary,source_outcome_id FROM reminders ORDER BY created_at,id`), triggers: await c.query(`SELECT id,name,prompt,chat_id,created_at,last_fired_at,fire_count FROM webhooks ORDER BY created_at,id`) }, "partial", "partially_restorable", ["Webhook authority is excluded. Restored schedules and triggers must remain disabled until owner review."]) },
