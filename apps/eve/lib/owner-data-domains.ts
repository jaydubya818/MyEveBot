...
export type OwnerDataPortability = "fully_restorable" | "restorable_with_reconnection" | "partially_restorable" | "reference_only" | "unavailable";
...
export const OWNER_DATA_DOMAINS: readonly OwnerDataDomain[] = [
  ...
  { id: "routines", name: "Routines & schedules", description: "Reminders, routines, safe trigger definitions, and review delivery preferences", exportable: true, restorable: false, deletable: true, sensitivity: "standard", dependencies: ["results"], required: true, load: async (c) => category(OWNER_DATA_DOMAINS_BY_ID.routines, { definitions: await ownerRows(c,`SELECT id,name,version,status,agent_id,configuration FROM execution_routines WHERE owner_id=$1 ORDER BY id`), versions: await ownerRows(c,`SELECT routine_id,version,configuration,review_binding,created_at FROM execution_routine_versions WHERE owner_id=$1 ORDER BY routine_id,version`), admissionHistory: await ownerRows(c,`SELECT id,routine_id,routine_version,scheduled_for,status,admission,preflight,run_id FROM execution_occurrences WHERE owner_id=$1 ORDER BY scheduled_for,id`), reminders: await c.query(`SELECT id,prompt,cron,timezone,next_fire_at,status,created_at,last_fired_at,routine_name,approval_boundary,source_outcome_id FROM reminders WHERE owner_id=$1 ORDER BY created_at,id`,[c.ownerId]), triggers: await c.query(`SELECT id,name,prompt,chat_id,created_at,last_fired_at,fire_count FROM webhooks ORDER BY created_at,id`) }, "partial", "partially_restorable", ["Webhook authority is excluded. Restored schedules and triggers must remain disabled until owner review."]) },
  ...
  { id: "files", name: "Files & artifacts", description: "Owner file and artifact metadata with explicit portability classification", exportable: true, restorable: false, deletable: true, sensitivity: "sensitive", dependencies: ["conversations", "runs"], required: false, load: async (c) => category(OWNER_DATA_DOMAINS_BY_ID.files, { chatFiles: (await ownerRows(c, `SELECT id,thread_id,filename,media_type,size_bytes,created_at FROM chat_files WHERE owner_id=$1 ORDER BY created_at,id`)).map((row) => ({ ...row, source: "chat", ownership: "owner", portability: "referenced", availability: "requires_source_storage" })), taskArtifacts: (await ownerRows(c, `SELECT a.id,a.task_id,a.check_id,a.specialist_role,a.kind,a.filename,a.content_type,a.size_bytes,a.sha256,a.redacted,a.created_at FROM task_artifacts a JOIN task_runs r ON r.id=a.task_id WHERE r.owner_id=$1 ORDER BY a.task_id,a.created_at,a.id`)).map((row) => ({ ...row, source: "run", ownership: "owner", portability: "referenced", availability: "requires_source_storage" })), computerArtifacts: (await ownerRows(c, `SELECT id,computer_session_id,action_id,run_id,kind,filename,content_type,size_bytes,sha256,created_at FROM computer_artifacts WHERE owner_id=$1 ORDER BY created_at,id`)).map((row) => ({ ...row, source: "computer", ownership: "owner", portability: "referenced", availability: "requires_source_storage" })) }, "partial", "reference_only", ["Binary file contents are not embedded in v1. Metadata and checksums do not by themselves constitute a file backup."]) },
  ...
  { id: "finance", name: "Finance", description: "Owner-entered receipt and purchase records", exportable: true, restorable: false, deletable: true, sensitivity: "sensitive", dependencies: [], required: false, load: async (c) => category(OWNER_DATA_DOMAINS_BY_ID.finance, { receipts: await c.query(`SELECT id,merchant,total_cents,currency,category,purchased_at,items,notes,logged_at FROM receipts ORDER BY purchased_at,id`) }) },
] as const;
