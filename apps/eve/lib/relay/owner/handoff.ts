...
import type { Environment,ExecutionCommand } from "./contracts.ts";
...
export class OwnerChannelHandoff {
 ...
 async accept(value:unknown){
  ...
  const matches=this.trust.mappings.filter(m=>m.enabled&&m.relayAccountId===work.accountId&&m.relayOwnerPrincipalId===work.ownerPrincipalId&&m.relayAgentId===work.agentId&&m.sourceIdentity===work.sourceIdentity);
  ...
  if(!agent||agent.status!=="active")throw new Error("Mapped Agent unavailable.");
  const requested=Date.parse(work.requestedAt),expiry=Date.parse(work.expiresAt);
  if(requested>Date.now()+5000||expiry<=Date.now()||expiry-requested>86400000||work.requestId!==work.taskId)throw new Error("Work lifetime or identity invalid.");
  ...
  const hash=ownerCommandHash(work),runId=`owner_run_${ownerCommandHash([work.accountId,work.requestId])}`;
  if(command.operation==="start"){
   ...
   try {await this.database.query(`WITH admitted AS (
    INSERT INTO task_runs(id,owner_id,kind,title,agent_id,status,max_duration_seconds,max_specialists,max_model_steps,max_retries_per_specialist,max_estimated_cost_usd)
    SELECT $1,$2,'delegated_work','Owner channel request',$3,'queued',LEAST(60,max_runtime_seconds),0,LEAST(8,max_steps),0,LEAST(0.1,max_estimated_cost_usd)
    FROM agents WHERE owner_id=$2 AND id=$3 AND status='active'
    ...
    VALUES($11,$4,$5,$2,$12,'start') ON CONFLICT(command_id) DO UPDATE SET payload_hash=
    CASE WHEN owner_channel_commands.owner_id=EXCLUDED.owner_id AND owner_channel_commands.relay_account_id=EXCLUDED.relay_account_id
      ...
      THEN owner_channel_commands.payload_hash ELSE NULL END`,[runId,mapping.ownerId,mapping.agentId,work.accountId,work.requestId,work.sourceIdentity,work.threadId,hash,JSON.stringify(work),work.expiresAt,command.commandId,ownerCommandHash(command)]);}
  ...
  const row=rows[0];
  if(!row)throw new Error("Canonical Run not admitted; status never starts work.");
  if(row.work_hash!==hash||row.agent_id!==mapping.agentId||row.source_identity!==work.sourceIdentity||row.revoked_at)throw new Error("Owner handoff binding changed or revoked.");
