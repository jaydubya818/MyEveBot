...
export function applyContextBudget(
  ...
  const tierScore: Record<ContextTier, number> = { hot: 2, warm: 1, cold: 0 };
  const optional = items.filter((item) => !item.mandatory).sort((a, b) =>
    tierScore[b.tier] - tierScore[a.tier] || b.score - a.score || a.id.localeCompare(b.id));
