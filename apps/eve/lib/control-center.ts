...
export async function getControlCenter(input: {
  ...
  const runs = rows.map((row): ControlRunView => {
    ...
    return {
      ...
      expectedOutput: nullableText(row.expected_output), taskStatus: status, view: controlViewForStatus(status),
      currentAction: nullableText(row.current_action), waitingReason: waitingReason(row, status), statusReason: nullableText(row.status_reason),
      agent: {
      ...
      approvalsPending: number(row.approvals_pending), startedAt: nullableIso(row.started_at), completedAt: nullableIso(row.completed_at),
  ...
  for (const row of countRows) {
    ...
    counts.all += count;
    counts[controlViewForStatus(text(row.status) as TaskStatus)] += count;
  }
