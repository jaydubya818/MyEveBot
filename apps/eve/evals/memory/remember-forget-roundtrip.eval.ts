...
function memoryIdFrom(output: unknown): string | null {
  if (output === null || typeof output !== "object" || Array.isArray(output)) return null;
  const record = output as Record<string, unknown>;
  const memoryId = record.id ?? record.documentId;
  return typeof memoryId === "string" && memoryId.length > 0 ? memoryId : null;
}
  ...
  async test(t) {
    ...
    try {
      ...
      if (documentId !== null) {
        ...
        await memoryStore.deleteForOwner(ownerId, documentId);
      }
    }
  },
});
