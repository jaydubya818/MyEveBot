# Sofie's phone — AgentPhone integration plan

Give Sofie a real phone number: send and receive SMS/MMS/iMessage, make and
...
- **Budget.** We set the webhook `timeout` to its 120s maximum and still emit an
  interim chunk immediately, because Sofie's turns routinely run long (computer
...
- **`openDM` builds a thread id that can never match an inbound one.** It emits
  ...
  conversation Sofie starts lives in a different thread from the reply.
...
- **`isMe: !isInbound` silently discards `agent.call_ended` for outbound and web
  calls**, so calls Sofie places produce nothing at all.
...
The narrow, honest version: tools that read Sofie's own inbound SMS.
...
Scope it to accounts **Sofie** creates, not the owner's — otherwise the agent loop
