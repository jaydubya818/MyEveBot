...
export function SolutionPackCatalogCard({
  ...
  return (
    <details className="group/solution rounded-xl border border-kumo-brand/25 bg-kumo-brand/5" open>
      <summary className="flex cursor-pointer list-none items-start gap-3 rounded-xl p-4 outline-none focus-visible:ring-2 focus-visible:ring-kumo-brand/50 [&::-webkit-details-marker]:hidden">
        ...
        <span className="mt-0.5 text-kumo-subtle transition-transform group-open/solution:rotate-90" aria-hidden>›</span>
      </summary>
      ...
      <div className="border-t border-kumo-brand/15 p-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {coordinator && <div className="flex flex-wrap gap-2"><Button size="sm" variant="primary" icon={PlayIcon} onClick={() => onUseSolutionPack(pack)}>Use Founder OS</Button><Button size="sm" variant="secondary" icon={PlusIcon} onClick={() => onCreateAgent(coordinator)}>Create Founder Agent</Button></div>}
        </div>
        ...
        <section className="mt-5" aria-labelledby={`${pack.id}-domains`}>
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {pack.domains.map((domain) => <details key={domain.id} className="rounded-xl border border-kumo-hairline bg-kumo-base p-3">
