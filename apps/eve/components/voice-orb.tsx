...
// runs a live GPT Realtime conversation as Sofie's voice and bridges real work
...
import { SofieDispatcher } from "@/lib/voice/dispatch";
...
export function VoiceOrb() {
  ...
  // Set when Sofie asks for a file. Browsers only open a file picker from a real
  ...
  const dispatcherRef = useRef<SofieDispatcher | null>(null);
  ...
  const handleAskSofie = useCallback(
  ...
  const handleAnswerSofie = useCallback(
      ...
      const realtime = new RealtimeVoiceSession({
        ...
        onFunctionCall: ({ callId, name, args }) => {
          ...
          if (name === "ask_sofie") {
            ...
            void handleAskSofie(generation, callId, request);
          } else if (name === "answer_sofie") {
            void handleAnswerSofie(generation, callId, typeof args.answer === "string" ? args.answer : "");
  ...
  }, [handleAnswerSofie, handleAskSofie, settleFunctionCall]);
  ...
  // and the command palette, so it stays reachable while watching Sofie work on
