...
export function DocumentationPanel() {
  ...
  useEffect(() => {
    const sync = () => {
      ...
      const exists = ADMIN_GUIDES.some((guide) => guide.id === id);
      setSelected(exists ? id : "start");
      setBrowsing(!exists);
    };
    sync();
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input id="guide-search" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setBrowsing(true); }} placeholder="Try approvals, Jev, backup, or connect…" className="w-full rounded-lg border border-kumo-hairline bg-kumo-canvas py-2.5 pe-3 ps-9 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-kumo-interact" />
        </div>
      ...
      {browsing && <nav aria-label="Documentation topics" className="grid gap-2 sm:grid-cols-2">
      ...
      {browsing && matches.length === 0 && <div className="rounded-xl border border-dashed border-kumo-hairline p-5"><p className="font-medium">No matching guides</p><p className="mt-1 text-sm text-kumo-subtle">Try a shorter search, or return to the full guide list.</p><button type="button" onClick={() => setQuery("")} className="mt-3 text-sm font-medium text-kumo-interact underline">Clear search</button></div>}
      {!browsing && <article aria-labelledby="guide-title" className="border-t border-kumo-hairline pt-6">
        <button type="button" onClick={() => { setBrowsing(true); window.history.pushState(null, "", window.location.pathname); requestAnimationFrame(() => document.getElementById("guide-search")?.focus()); }} className="mb-5 text-sm font-medium text-kumo-interact underline underline-offset-4">Back to all guides</button>
