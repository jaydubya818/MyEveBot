...
import { BUILTIN_ROLE_CATALOG, BUILTIN_SOLUTION_PACKS } from "@/lib/builtin-role-catalog";
...
function RoleCatalogPanel({ onCreateAgent, onUseRole }: { onCreateAgent: (role: RoleDefinition) => void; onUseRole: (role: RoleDefinition) => void }) {
  ...
  return <section className="mt-8 border-t border-kumo-hairline pt-6" aria-labelledby="role-catalog-title">
    ...
    <div className="mt-4 grid gap-4">
      {visibleRolePacks.map((pack) => <details key={pack.id} className={cn("group/pack rounded-xl border border-kumo-hairline bg-kumo-tint/40", pack.id === "verification" && "order-last")} open={pack.id === "marketing-engineering"}>
      ...
      {BUILTIN_SOLUTION_PACKS.map((pack) => <SolutionPackCatalogCard key={pack.id} pack={pack} roleCatalog={BUILTIN_ROLE_CATALOG} onCreateAgent={onCreateAgent} onUseRole={onUseRole} />)}
    </div>
  </section>;
}
