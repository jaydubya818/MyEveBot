...
import { DecisionExperimentConclusions } from "./decision-experiment-conclusions";
import { DecisionExperimentPanel } from "./decision-experiment-panel";
...
export function DecisionIntelligencePanel() {
  ...
  return (
    ...
    >
      ...
      <DecisionExperimentConclusions />
      {data.run?.experiment ? (
        <DecisionExperimentPanel
          data={data}
          query={query}
          select={select}
          threshold={threshold}
        ...
        <>
          <section className={box}>
            ...
            <p className="mt-1 text-sm text-kumo-subtle">
              6 of 7 Knowledge types evaluated. Insight remains handled
              exclusively by MyEve’s canonical Knowledge system and is outside
              this experiment.
            </p>
          ...
          {!data.run && (
            <section className={box}>
              <h3 className="font-medium">
                No Decision Intelligence evidence yet
              </h3>
              <p className="mt-2 text-sm text-kumo-subtle">
                The experiment is not enabled by default. Synthetic benchmarks
                run through an explicitly invoked developer command. Live shadow
                evaluation is off.
              </p>
            </section>
          )}
          {data.run && m && (
            <>
              ...
              <section className={box}>
                ...
                <p className="mt-2 text-sm text-kumo-subtle">
                  Jev confidence is the reported probability of the selected
                  class. Missing probabilities remain unavailable. Compare this
                  signal with correctness before considering any future use.{" "}
              ...
              <section className={box}>
                ...
                <p className="mt-2 text-sm text-kumo-subtle">
                  This control only explores the evidence. It cannot change
                  runtime behavior. Missing or failed predictions fall back.
                </p>
                <label
              ...
              <section className={box}>
                <h3 className="font-medium">
                  Recent decisions and disagreements
                </h3>
                ...
                <ul className="mt-3 divide-y divide-kumo-hairline">
                  {data.rows.map((row) => (
                    <li key={row.id} className="py-3 text-xs">
                      ...
                      <p className="mt-1">
                        Provider: {row.result?.outcome ?? row.failure} ·
                        Confidence: {percent(row.result?.confidence)}
                      </p>
                    </li>
                  ))}
                </ul>
              ...
              {data.detail && (
                <section className={box} aria-labelledby="decision-detail">
                  ...
                  <p className="mt-3 text-xs">
                    ...
                    {data.detail.evidence.canonical ?? "Not measured"} ·
                    Provider:{" "}
                  ...
                  <dl className="mt-3 space-y-2 text-xs">
                    {OUTCOMES.map((outcome) => (
                      <div key={outcome} className="flex justify-between">
                        ...
                        <dd>
                          {percent(
                            data.detail!.evidence.result?.probabilities?.[
                              outcome
                            ],
                          )}
                        </dd>
                  ...
                  <p className="mt-3 text-xs text-kumo-subtle">
                    ...
                    {data.detail.evidence.result?.evaluatedAt}. SHADOW ·
                    Influence: None.
                  </p>
                  <Button
                    className="mt-3"
              ...
              <details className={box}>
                ...
                <dl className="mt-3 space-y-2 break-all text-xs">
                  <Metric
                    label="Contract"
                    value="knowledge.classification:v1"
                  />
