...
interface RoutineData {executionReady:boolean;routines:RoutineRow[];agents:AgentOption[];capabilities:{id:string;name:string}[]}
...
function RoutineReview({routine,data,onSaved,onClose}:{routine:RoutineRow;data:RoutineData;onSaved:()=>void;onClose:()=>void}) {
  ...
  async function save(event:React.FormEvent) {
    ...
    const configuration:RoutineConfiguration={instructions:routine.prompt,authority:{allowedCapabilities:capabilities,allowedTargets:[],maximumRisk:"low",requiresApprovalFor:[]},
  ...
  return <form onSubmit={save} className="mt-4 space-y-4 border-t border-kumo-hairline pt-4" aria-label={`Review ${routine.routine_name??"reminder"}`}>
    ...
    <fieldset><legend className="text-sm font-medium">Allowed capabilities</legend>
      <p className="mt-1 text-xs text-kumo-subtle">Only selected reads are permitted. External writes and delegation are blocked. No selection allows a reminder with no tool access.</p>
    ...
    <label className="block text-sm">Result notification<select className={field} value={channel} onChange={e=>setChannel(e.target.value as typeof channel)}>
      <option value="in_app">In MyEve</option><option value="push">Web push to my devices</option><option value="telegram">Telegram to my configured account</option>
    </select></label>
...
export function RoutinesPanel() {
  ...
  return <section className="mb-8" aria-labelledby="routines-title">
    ...
    {data&&!data.executionReady&&<p className="mt-3 text-sm text-kumo-subtle">Unattended execution is blocked while tool authorization is being completed. You can save your review now; no routine will run yet.</p>}
    ...
    {error?<div role="alert" className="mt-3 text-sm">{error}<Button className="ml-3" size="sm" variant="secondary" onClick={()=>void load()}>Retry</Button></div>:!data?<div className="py-6" role="status" aria-label="Loading routines"><Loader size={18}/></div>:data.routines.length===0?
      ...
      <ul className="mt-4 space-y-3">{data.routines.map(r=>{
        ...
        return <li key={r.id} className="rounded-xl border border-kumo-hairline p-4">
          ...
          {reviewing===r.id&&<RoutineReview routine={r} data={data} onClose={()=>setReviewing(null)} onSaved={()=>{setReviewing(null);setNotice(data.executionReady?"Routine approved. Missed recurring runs will not be replayed.":"Owner review saved. Execution remains blocked until tool authorization is ready.");void load();}}/>}
        </li>;
      })}</ul>}
  </section>;
}
