...
import { ArrowClockwiseIcon, ArrowSquareOutIcon, CheckCircleIcon, ClockIcon, StopIcon } from "@phosphor-icons/react";
...
export function TaskRunCard({ threadId }: { threadId: string }) {
  ...
  const isQa = task.kind === "product_qa";
  ...
  async function retry() {
    ...
    setUpdating(true);
    try {
      const response = await fetch(`/api/task-runs/${taskId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "retry" }) });
      if (response.ok) setTask(((await response.json()) as { task: TaskRunView }).task);
    ...
    <MessageTaskShell>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-kumo-subtle">{isQa ? `${completeSpecialists}/${task.guardrails.maxSpecialists} specialists · ${passed}/${task.checks.length} checks · ` : task.expectedOutput ? `Deliverable: ${task.expectedOutput} · ` : ""}{task.usage.modelSteps}/{task.guardrails.maxModelSteps} steps · ${task.usage.estimatedCostUsd.toFixed(2)}/${task.guardrails.maxEstimatedCostUsd}</p>
        </div>
        <Badge variant={statusVariant(task.status)}>{task.status.replace("_", " ")}</Badge>
      </div>
      {isQa && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-kumo-recessed" aria-hidden>
        <div
      ...
      {!isQa && task.resultSummary && <p className="mt-3 text-sm leading-6">{task.resultSummary}</p>}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <span className="flex items-center gap-1.5 text-xs text-kumo-subtle">
          ...
          {Math.round(task.guardrails.maxDurationSeconds / 60)}-minute {isQa ? "Balanced QA" : "bounded work"} run
        </span>
        <div className="flex gap-2">
          ...
          {task.status === "failed" && <Button variant="secondary" size="sm" icon={ArrowClockwiseIcon} disabled={updating} onClick={() => void retry()}>Retry</Button>}
          <a
          ...
          >
            {isQa ? "Review evidence" : "Review result"}
