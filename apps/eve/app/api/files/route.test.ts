...
vi.mock("@/lib/web-auth", () => ({
  ...
  webPrincipal: mocks.webPrincipal,
}));
...
describe("/api/files owner scoping", () => {
  beforeEach(() => {
    ...
    mocks.webPrincipal.mockReturnValue(owner);
  ...
  it("scopes file listing to the app's stable owner", async () => {
    const response = await GET(new Request("https://sofie.example/api/files"));
