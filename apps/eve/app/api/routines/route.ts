...
export async function GET(request:Request):Promise<Response> {
  ...
  try {
    ...
    const admission=new RoutineAdmission();
    const readyRoutines=await Promise.all(routines.map(async r=>({...r,readiness:r.execution_routine_id?await admission.inspect(ownerId,String(r.execution_routine_id)):null})));
    return Response.json({executionReady:ROUTINE_EXECUTION_READY,routines:readyRoutines,agents:agents.map(a=>({id:a.id,name:a.name,status:a.status,limits:a.limits})),capabilities:ROUTINE_CAPABILITIES.map(id=>({id,name:getCapability(id)?.name??id,risk:getCapability(id)?.risk.level??"critical"}))},{headers:{"Cache-Control":"no-store"}});
