...
export async function GET(request:Request):Promise<Response> {
  ...
  try {
    ...
    return Response.json({executionReady:ROUTINE_EXECUTION_READY,routines,agents:agents.map(a=>({id:a.id,name:a.name,status:a.status,limits:a.limits})),capabilities:ROUTINE_CAPABILITIES.map(id=>({id,name:getCapability(id)?.name??id}))},{headers:{"Cache-Control":"no-store"}});
...
export async function POST(request:Request):Promise<Response> {
  ...
  try {
    ...
    const safe=/^(Choose an active Agent|This capability is not yet|A selected capability is unavailable|Routine limits cannot exceed|Reminder changed\.|Target restrictions and per-action approvals)/.test(message);
