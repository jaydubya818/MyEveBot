...
function configuredUrl(): string {
  const value = nonEmptyEnv("SOFIE_LOCAL_MCP_URL") ?? LOCAL_DEFAULT_URL;
  return parseLocalComputerMcpUrl(value).href;
}

function requiredToken(): string {
  const token = nonEmptyEnv("SOFIE_LOCAL_MCP_TOKEN");
  if (token === undefined || token.length < 32) {
    throw new Error(
      "Sofie Local is not paired. Set SOFIE_LOCAL_MCP_TOKEN to the token printed by `npm run local:setup`.",
    );
  }
  return token;
}
...
function cloudflareAccessHeaders(): Record<string, () => string> {
  const clientId = nonEmptyEnv("SOFIE_LOCAL_CF_ACCESS_CLIENT_ID");
  const clientSecret = nonEmptyEnv("SOFIE_LOCAL_CF_ACCESS_CLIENT_SECRET");
  if (clientId === undefined && clientSecret === undefined) return {};
  if (clientId === undefined || clientSecret === undefined) {
    throw new Error(
      "Set both SOFIE_LOCAL_CF_ACCESS_CLIENT_ID and SOFIE_LOCAL_CF_ACCESS_CLIENT_SECRET, or neither.",
    );
  }
  return {
