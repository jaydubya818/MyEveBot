...
description: >-
  Analyzes a working, staged, saved, or branch git diff against a one-line
  intent. Use when the user asks whether a change grew beyond the intended fix,
  whether a PR is too broad, what unrelated files they touched, or wants
  evidence-backed keep, split, or justify guidance. Operates locally, offline,
  and read-only.
license: Apache-2.0
metadata:
...
> **MyEve adaptation:** The routing description was narrowed to scope-review
> intent so routine formatting, lint, and commit-writing requests do not claim
> this skill. The classifier and workflow remain upstream-compatible.
