...
Other project skills describe what good looks like: review workflows inspect the change, `tdd` supplies a cycle, and `security-and-hardening` supplies a threat list. Those instructions still need a durable, project-specific bar that survives the end of the session.
...
- The user wants a code review right now or a CI pipeline built; use a focused review or automation workflow instead
...
- `interrogate` — adversarial review; this skill decides what the review enforces
- `fix-ci` — restoring failing checks once these constraints run in CI
