...
description: >-
  Creates an approval-gated echo brief before Sofie acts on a ramble. Use only
  when the user explicitly says "let me think out loud", asks to ramble before
  building, asks to be interviewed about a fuzzy idea, or invokes this skill by
  name. Capture the mission, locked decisions, open questions, reversals, parked
  tangents, and clearly separated model inferences. Do not infer activation from
  message length, dictation artifacts, or weak punctuation alone.
license: Apache-2.0
metadata:
  activation: "explicit"
...
> **MyEve adaptation:** Sofie activates this skill only after an explicit user
> request. The upstream auto-trigger examples remain useful once the skill is
> active, but message shape or voice dictation alone does not opt the user in.
