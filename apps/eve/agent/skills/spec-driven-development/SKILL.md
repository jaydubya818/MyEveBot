...
Execute tasks one at a time with the installed `principle-sequence-verifiable-units` and `tdd` skills. Use `principle-guard-the-context-window` to load only the spec sections and source files needed for the current task rather than flooding the agent with the entire spec.
