...
description: >-
  Diagnoses why dependencies will not install by inspecting one named
  requirements.txt, pyproject.toml, or package.json for direct-manifest
  problems: standard-library shadowing pins, abandoned backports, unpinned
  dependencies, duplicates, and conflicting exact pins. Use for a dependency
  manifest check, dependency autopsy, or suspected manifest rot. Runs locally
  and offline by default; PyPI yanked-release checks require explicit approval.
license: Apache-2.0
metadata:
...
> **MyEve adaptation:** The routing description was narrowed for Sofie's
> combined skill catalog. The diagnostic procedure and executable remain
> upstream-compatible.
