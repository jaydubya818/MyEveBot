...
function localComputerConfigured(): boolean {
  return Boolean(
    process.env.SOFIE_LOCAL_MCP_URL?.trim() && process.env.SOFIE_LOCAL_MCP_TOKEN?.trim(),
  );
}
...
- \`trash_path\` moves items into a recoverable \`.sofie-trash\` folder; it
...
- If the connection is offline, say that the Mac may be asleep or Sofie Local
