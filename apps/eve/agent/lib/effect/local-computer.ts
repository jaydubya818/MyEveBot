...
export const LocalComputerTaskInput = Schema.Struct({
  instruction: Schema.String.check(
    ...
    description:
      "Complete goal for Sofie's real Mac, including any consequential action the owner explicitly authorized.",
  }),
...
export function describeLocalComputerError(error: LocalComputerError): string {
  switch (error.reason) {
    case "not_configured":
      return `Sofie Local is not configured: ${error.detail}`;
    case "connection":
      return `Sofie Local is offline: ${error.detail}`;
    case "tool":
      return `Sofie Local refused the operation: ${error.detail}`;
    case "model":
...
export function localComputerConfigured(): boolean {
  return (
    env("SOFIE_LOCAL_MCP_URL") !== undefined &&
    (env("SOFIE_LOCAL_MCP_TOKEN")?.length ?? 0) >= 32
  );
}

function localModel(): string {
  const model = env("SOFIE_LOCAL_COMPUTER_MODEL") ?? DEFAULT_LOCAL_MODEL;
  if (!MODEL_ID.test(model)) {
    throw new LocalComputerError({
      reason: "not_configured",
      detail: "SOFIE_LOCAL_COMPUTER_MODEL must look like provider/model.",
    });
  }
  return model;
}
...
function taskTimeoutSeconds(): number {
  const raw = env("SOFIE_LOCAL_TASK_TIMEOUT_SECONDS");
  if (raw === undefined) return DEFAULT_TASK_TIMEOUT_SECONDS;
  const parsed = Number(raw);
  if (!Number.isSafeInteger(parsed) || parsed < 10 || parsed > 600) {
    throw new LocalComputerError({
      reason: "not_configured",
      detail: "SOFIE_LOCAL_TASK_TIMEOUT_SECONDS must be an integer from 10 to 600.",
    });
  }
  return parsed;
}
...
function connectionConfig(): { url: string; headers: Record<string, string> } {
  const rawUrl = env("SOFIE_LOCAL_MCP_URL");
  const token = env("SOFIE_LOCAL_MCP_TOKEN");
  if (rawUrl === undefined || token === undefined || token.length < 32) {
    throw new LocalComputerError({
      reason: "not_configured",
      detail: "set SOFIE_LOCAL_MCP_URL and a token of at least 32 characters.",
    });
  }
  let url: URL;
  try {
  ...
  const accessId = env("SOFIE_LOCAL_CF_ACCESS_CLIENT_ID");
  const accessSecret = env("SOFIE_LOCAL_CF_ACCESS_CLIENT_SECRET");
...
async function createClient(): Promise<MCPClient> {
  ...
  try {
    return await createMCPClient({
      clientName: "sofie-local-computer-use",
      maxRetries: 1,
      transport: {
        type: "http",
        url: config.url,
