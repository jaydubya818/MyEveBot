...
describe("anonymous Eve rate limit", () => {
  ...
  it("counts message creation and continuation but not control routes", () => {
    expect(
      isAnonymousEveMessageRequest(
        new Request("https://sofie.example/eve/v1/session", { method: "POST" }),
      ),
    ).toBe(true);
    expect(
      isAnonymousEveMessageRequest(
        new Request("https://sofie.example/eve/v1/session/session-1", { method: "POST" }),
      ),
    ).toBe(true);
    expect(
      isAnonymousEveMessageRequest(
        new Request("https://sofie.example/eve/v1/session/reset", { method: "POST" }),
      ),
    ).toBe(false);
    expect(
      isAnonymousEveMessageRequest(
        new Request("https://sofie.example/eve/v1/session/session-1/cancel", {
          method: "POST",
        }),
      ),
    ).toBe(false);
    expect(
      isAnonymousEveMessageRequest(
        new Request("https://sofie.example/eve/v1/session/session-1/stream"),
      ),
    ).toBe(false);
  });
  ...
  it("uses the Vercel-authenticated client address without storing the raw IP", () => {
    const first = anonymousEveCallerHash(
      new Request("https://sofie.example/eve/v1/session", {
        headers: {
