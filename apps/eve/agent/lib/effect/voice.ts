...
export const ANSWER_SOFIE_TOOL = {
  type: "function",
  name: "answer_sofie",
  description:
    "When an ask_sofie result says the backend is waiting for the user's confirmation or input (status needs_input), relay the question aloud, get the user's spoken answer, and pass that answer here verbatim.",
  parameters: {
    type: "object",
    properties: {
...
export function buildVoicePersona(
  ...
  const memoryBlock =
    profile === null
      ? `Your long-term memory profile is unavailable right now; rely on the conversation and ask_sofie.`
...
# Doing real work
ask_sofie reaches your own full self — the same assistant ${owner} uses in the chat app, with every tool, skill, connected app, and memory you have there, including anything added since. Voice is just another way to reach you, so there is nothing ${owner} can ask in chat that he cannot ask here.
You handle light conversation yourself. For anything else — actions, lookups, email, calendar, purchases, reminders, memory updates, anything needing tools or current data — call ask_sofie with one clear, self-contained request written with all the context it needs. It receives a copy of the recent spoken lines as well, but it cannot hear the call — write the request so it stands on its own.
- Never say you cannot do something because you are "just the voice". If you are unsure whether it is possible, ask_sofie and find out.
- Before calling ask_sofie for a non-trivial task, say a very short acknowledgment first, like "on it" or "give me a second".
- NEVER invent results. Only report what ask_sofie actually returned. If it failed, say so plainly and offer the next step.
- While a request runs you may be told to briefly narrate progress; keep those to a few words.
- If the result says the backend needs the user's confirmation or input (status needs_input), read the question to ${owner}, then pass the spoken answer through answer_sofie.
...
function safeProfile(): Effect.Effect<MemoryProfile | null> {
  ...
  return Effect.tryPromise(() => memoryStore.listForOwner(ownerId)).pipe(
    Effect.map((memories) => ({
      static: memories.filter((memory) => memory.permanent).slice(0, 20).map((memory) => memory.content),
      dynamic: memories.filter((memory) => !memory.permanent).slice(0, 20).map((memory) => memory.content),
    })),
  ...
  return Effect.gen(function* () {
    ...
    const body = {
      ...
      session: {
        ...
        tools: [ASK_SOFIE_TOOL, ANSWER_SOFIE_TOOL, STOP_TASK_TOOL, REQUEST_FILE_TOOL],
        tool_choice: "auto",
      },
    };
