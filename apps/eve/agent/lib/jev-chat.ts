...
export const jevChatInput = z.discriminatedUnion("operation", [
  z.object({ operation: z.literal("status") }).strict(),
  z.object({ operation: z.literal("evaluate"), statements: z.array(z.string().trim().min(1).max(2000)).min(1).max(5) }).strict(),
]);
...
export async function evaluateWithJev(value: Input, ctx: Context) {
  ...
  if (input.statements.some(hasSecret)) return { ...metadata, status: "denied", code: "PRIVACY_EXCLUDED", usedJev: false };
  ...
  return { ...metadata, status: results.length === input.statements.length && results.every(r => r.status === "evaluated") ? "evaluated" : "incomplete",
    usedJev: results.some(r => r.status === "evaluated"), results,
    requestedCount: input.statements.length,
