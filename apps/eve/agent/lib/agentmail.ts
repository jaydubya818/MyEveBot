...
// threading. Sofie owns one inbox; this module is the only place that talks to
...
function desiredUsername(inboxes: Inbox[]): string | undefined {
  ...
  // Moving domains should keep the local part (sofie@agentmail.to →
  // sofie@example.com), so reuse the default inbox's username when it exists.
...
/** The agent's own email address, e.g. "sofie@agentmail.to". */
