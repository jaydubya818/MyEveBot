...
integration("scoped memory and Context Assembly prove the seeded Agent/Goal isolation path", async (t) => {
  ...
  try {
    ...
    const researcherContext = { ownerId, agentId: researcher.id, goalId: adobe.id, taskId: task.id };
    ...
    await memoryStore.add("Personal spending analysis convention.", {
      context: { ownerId, agentId: finance.id },
      scope: { type: "agent", id: finance.id },
    });
    await memoryStore.add("Vacation dates and hotel shortlist.", {
      context: { ownerId, agentId: researcher.id, goalId: unrelated.id },
    ...
    assert.ok(result.sourceRefs.includes("conversation:recent-native"));
    ...
    assert.deepEqual(await memoryStore.search("all context", {
      ownerId: otherOwnerId, agentId: otherPrimary.id,
    }), []);
    ...
    await db().query(`DELETE FROM memory_scope_migrations WHERE owner_id IN ($1,$2)`, [ownerId, otherOwnerId]).catch(() => undefined);
