...
globalThis.fetch=async()=>{throw new Error('Cloud providers forbidden during migration qualification');};
const connection = { host:'127.0.0.1',port:55442,user:'myeve_test' };
const admin = new Client({...connection,database:'postgres'});
await admin.connect();
const created = [], clients = [];
const oldRunnerDirectory=await mkdtemp(tmpdir()+'/myeve-historical-runner-');
...
try {
  ...
  for (const [db,before] of [[a,stateA],[b,stateB]]) {
    ...
    for(const session of after.computer_sessions) {
      ...
      for(const key of Object.keys(previous).filter(k=>!['status','failure_code','failure_summary','completed_at'].includes(k)))assert.deepEqual(session[key],previous[key],key+' preserved');
    }
    for(const control of after.computer_control_leases){assert.equal(control.controller,'NONE');assert.equal(control.owner_input_enabled,false);}
    assert.equal((await db.client.query('SELECT count(*) FROM computer_resource_lifecycles')).rows[0].count,'0','no historical resource ownership invented');
  }
  ...
  // Execute the exact qualified b36f016 runner to construct real reconciled
  // origins, then prove the new bridge preserves both its ledger and receipt.
  const originalRunner=execFileSync('git',['show','b36f0165dc0d6ee20eb64a092dc80c58c99a4c3b:apps/eve/scripts/migration-runner.ts'],{encoding:'utf8'});
  const oldRunnerSource=originalRunner.replace('"./migration-sql.ts"',JSON.stringify(new URL('../scripts/migration-sql.ts',import.meta.url).href))
    .replace('"./deployed-lineage-396631a.json"',JSON.stringify(new URL('../scripts/deployed-lineage-396631a.json',import.meta.url).href));
  const oldRunnerFile=oldRunnerDirectory+'/runner.ts';await writeFile(oldRunnerFile,oldRunnerSource);
  const oldRunner=await import(pathToFileURL(oldRunnerFile).href);
  const historicalName='0031_deployed_lineage_reconciliation.sql';
  const historicalSql=await readFile(new URL('../scripts/historical-migrations/'+historicalName,import.meta.url),'utf8');
  const oldManifest=[...migrations.filter(m=>m.name<'0031'),{name:historicalName,checksum:digest(historicalSql),statements:splitSqlStatements(historicalSql)}];
  for(const origin of ['canonical','feature']) {
    const prior=await build(origin,'reconciled_'+origin);
    await oldRunner.runMigrations(prior.db,oldManifest,quiet);
    const oldLedger=(await prior.client.query('SELECT * FROM sofie_schema_migrations ORDER BY name')).rows;
    const oldReceipt=(await prior.client.query('SELECT * FROM sofie_migration_reconciliations')).rows;
    await runMigrations(prior.db,migrations,quiet);await runMigrations(prior.db,migrations,quiet);
    assert.deepEqual((await prior.client.query('SELECT * FROM sofie_schema_migrations WHERE name=ANY($1::text[]) ORDER BY name',[oldLedger.map(r=>r.name)])).rows,oldLedger);
    assert.deepEqual((await prior.client.query('SELECT * FROM sofie_migration_reconciliations')).rows,oldReceipt);
    assert.equal(digest(dump(prior.name)),digest(dump(a.name)),origin+' reconciled lineage converges structurally');
    const bridge=(await prior.client.query('SELECT * FROM sofie_migration_bridge_receipts')).rows[0];
    assert.equal(bridge.satisfied_migrations[historicalName],undefined,'executed history is not falsely satisfied');
    await prior.client.query("UPDATE sofie_migration_bridge_receipts SET canonical_manifest='{}'");
    await assert.rejects(runMigrations(prior.db,migrations,quiet),/bridge evidence/);
  }
  const lazy=await build('canonical','lazy');
  await runMigrations(lazy.db,migrations.filter(m=>m.name<'0033'),quiet);
  await lazy.client.query(`INSERT INTO computer_template_preparations(id,scope,fingerprint,provider,state,deadline,template_id)
    VALUES('owned-preparation','owner-scope','fingerprint','vercel','READY',now()+interval '10 minutes','synthetic-snapshot')`);
  await lazy.client.query(`INSERT INTO computer_resource_lifecycles(id,owner_id,agent_id,run_id,computer_session_id,runtime_session_id,provider,environment,resource_name,generation,provision_id,preparation_id,source_snapshot_id,state)
    VALUES('owned-lifecycle','owner','ava','run','stale-computer','synthetic-runtime','vercel','synthetic-environment','synthetic-resource',1,'synthetic-provision','owned-preparation','synthetic-snapshot','active')`);
  const bindings=(await lazy.client.query('SELECT * FROM computer_resource_lifecycles')).rows;
  const preparations=(await lazy.client.query('SELECT * FROM computer_template_preparations')).rows;
  await runMigrations(lazy.db,migrations,quiet);await runMigrations(lazy.db,migrations,quiet);
  assert.deepEqual((await lazy.client.query('SELECT * FROM computer_resource_lifecycles')).rows,bindings,'existing durable bindings preserved');
  assert.deepEqual((await lazy.client.query('SELECT * FROM computer_template_preparations')).rows,preparations);
  ...
  await admin.end();
  await rm(oldRunnerDirectory,{recursive:true,force:true});
}
