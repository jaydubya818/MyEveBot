...
for(const width of [1440,390])test(`peer policy owner review, save, reload, revoke and mobile layout at ${width}px`,async({page})=>{
  ...
  await page.route('**/api/relay/peer-permissions',async route=>{
    ...
    }else await route.fulfill({json:{relationships,discoveryStatus:'AVAILABLE',peers:[{address:'relay://atlas/research',name:'Atlas'}]}});
  });
  ...
  await panel.getByLabel('Exact resource',{exact:true}).fill('relay://atlas/research');
